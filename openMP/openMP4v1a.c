...
int main(int argc, char **argv) {
	FILE *out_file = fopen("openMPv1a.txt", "w"); // write only
	...
	#pragma omp parallel for num_threads(nthreads) private(id) shared(size, vetor)
	for(int i = 0; i < size; i++){
		id = omp_get_thread_num();
		int aux = vetor[i];
		vetor[i] = square(vetor[i]);
		//printf("Apenas para controle, Thread %i indice %i\n", id, i);
		// write to file vs write to screen 
        //printf(file, "this is a test %d\n", integer); // write to file
        fprintf(out_file, "Apenas para controle, Thread %i indice %i valor %i raiz aproximada %i\n", id, i, aux, vetor[i]); // write to file
	}
